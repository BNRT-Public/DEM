...
105     FORMAT((A21),',',<NPRINT-1>(3X,A21,','))
        PR05=1
    END IF
...
111     FORMAT((A15),',',12(3X,A15,','))
    ...
    IF (IBCO.GT.0)THEN
        IF (ALLOCATED(DATAPRINT)) DEALLOCATE(DATAPRINT)
...
116     FORMAT((A15),',',<NPRINT-1>(3X,A15,','))
    ...
    ! ARCHIVO CON VALORES DE FUERZA EN LAS CONDICIONES DE BORDE A CADA TIEMPO
    ! FILE: FORCE_Txx.dat
    IF (OU17.GT.0)THEN
        IF (ALLOCATED(DATAPRINT)) DEALLOCATE(DATAPRINT)
        DATFILE = 'LOAD_T'//NUMTEST//'.dat'
...
117     FORMAT((A15),',',<NPRINT-1>(3X,A15,','))
    ...
    ! ARCHIVO CON VALORES DE DESPLAZAMIENTO EN LAS CONDICIONES DE BORDE A CADA TIEMPO
    ! FILE: BOUND_Txx.dat
    IF (OU18.GT.0)THEN
        IF (ALLOCATED(DATAPRINT)) DEALLOCATE(DATAPRINT)
        DATFILE = 'BOUND_T'//NUMTEST//'.dat'
        OPEN (UNIT=18,ACCESS='SEQUENTIAL',FILE=DATFILE,STATUS='UNKNOWN')
        ...
        WRITE (18,118) DATAPRINT
118     FORMAT((A15),',',<NPRINT-1>(3X,A15,','))
