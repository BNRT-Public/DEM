    ...
    !!$OMP PARALLEL DO PRIVATE(I,AUX,CNTEMP,BSEL,STRBSEL,STRBCAL,STRCAVG,KRTEMP) SHARED(STR,DSTR,EM,EP,KR,ECOMP,ETRAC,CON,FS,DM,FORCE,CN)
    DO I=1,NBT  
...
40      CONTINUE
        ...
        IF ((STR(I).GT.0.0).AND.(.NOT.FS(I))) THEN
            CNTEMP(1:2,:)=CN.EQ.CN(1,I)
            CNTEMP(3:4,:)=CN.EQ.CN(2,I)
            BSEL=ANY(CNTEMP,1)
            BSEL(I)=.FALSE.
            STRBSEL=PACK(STR,BSEL)
            ...
            IF (COUNT(STRBCAL).GT.0) THEN
                ...
                IF (EM(1,I).GT.EP(1,I)) THEN
                    KRTEMP=KR(1,I)*(-30.0*STRCAVG/EP(1,I))
